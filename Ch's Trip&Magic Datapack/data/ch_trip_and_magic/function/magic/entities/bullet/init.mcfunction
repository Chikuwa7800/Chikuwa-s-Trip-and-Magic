## 状態決め
 ...
 # エンティティデータにattribute的なのを代入
  $data merge entity @s {teleport_duration:1,shadow_radius:0.1,shadow_strength:1,data:{"speed":"$(speed)","damage":"$(damage)","damage_type":"$(damage_type)","gravity":"$(gravity)","rerun":"$(rerun)","hit_ground_effect":"$(hit_ground_effect)","hit_effect":"$(hit_effect)","tick":"$(tick)","addition_effect":"$(addition_effect)"}}
 # 弾丸の向きをオーナーと一致させる
  rotate @s ~ ~
