execute if entity @s[tag=ch_tm_magic_volt_cooldown] run function ch_trip_and_magic:magic/cooldown {"name":"item_skill.magic.bolt.lv1"}
...
execute if entity @s[scores={ch_tm_magic_mana=..7}] run tellraw @s {"translate":"notice.notenough_mana","color":"dark_red","bold":true,"italic":false,"underlined":true}
